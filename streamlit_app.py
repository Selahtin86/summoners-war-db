...
def load_data() -> pd.DataFrame:
    ...
    if "source_order" in df.columns:
        df["source_order_num"] = pd.to_numeric(df["source_order"], errors="coerce")
    else:
        df["source_order_num"] = range(1, len(df) + 1)
    return df
    ...
    for column, monster in zip(columns, rows[start : start + cards_per_row]):
        with column:
            with st.container(border=True):
                ...
                details = [f"{icon} {element}".strip() if element else "", family, f"Nat {stars}" if stars else ""]
